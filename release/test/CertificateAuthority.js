...
require('chai').should();
var Q = require('q');
var CA = require('../CertificateAuthority');
describe('CertificateAuthority', function () {
    ...
    describe('#constructor()', function () {
        context('when 1st CA', function () {
            ...
            it('should have private key', function () {
                return ca.caCertificate.then(function (caCert) {
                    caCert.privateKey.indexOf('-----BEGIN PRIVATE KEY-----').should.be.equal(0);
                    caCert.privateKey.indexOf('-----END PRIVATE KEY-----').should.be.above(0);
                });
            });
            it('should have CA certificate', function () {
                return ca.caCertificate.then(function (caCert) {
                    caCert.certificate.indexOf('-----BEGIN CERTIFICATE-----').should.be.equal(0);
                    caCert.certificate.indexOf('-----END CERTIFICATE-----').should.be.above(0);
                });
            });
        ...
        context('when 2nd CA from same common name', function () {
            it('should load existing CertificateAuthority', function () {
                ...
                return Q.all([ca.caCertificate, newCA.caCertificate]).spread(function (caCert, newCACert) {
    ...
    describe('#sign()', function () {
        ...
        context('when 1st certificate', function () {
            it('should return certificate', function () {
                return ca.sign('*.test1.com', 'DNS: *.test1.com, DNS: test1.com').then(function (certificate) {
                    ...
                    certificate.indexOf('-----END CERTIFICATE-----').should.be.above(0);
                });
            });
            ...
            it('should create CA serial number file', function () {
                return Q.nfcall(fs.readFile, 'keys/TestCA-CA-cert.srl').then(function (data) {
                    serial = '' + data;
        ...
        context('when 2nd certificate', function () {
            it('should return certificate', function () {
                return ca.sign('*.test2.com', 'DNS: *.test2.com, DNS: test2.com').then(function (certificate) {
                    certificate.indexOf('-----BEGIN CERTIFICATE-----').should.be.equal(0);
                    certificate.indexOf('-----END CERTIFICATE-----').should.be.above(0);
                });
            });
